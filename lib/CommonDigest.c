...
CCDigestGetDigestInfo(CCDigestAlgorithm algorithm) {
    ...
    dispatch_once(&globals->digest_info_init, ^{
        ...
        globals->digest_info[kCCDigestMD2] = &ccmd2_ltc_di;
        globals->digest_info[kCCDigestMD4] = &ccmd4_ltc_di;
        globals->digest_info[kCCDigestMD5] = ccmd5_di();
        globals->digest_info[kCCDigestRMD128] = &ccrmd128_ltc_di;
        globals->digest_info[kCCDigestRMD160] = &ccrmd160_ltc_di;
        globals->digest_info[kCCDigestRMD256] = &ccrmd256_ltc_di;
        globals->digest_info[kCCDigestRMD320] = &ccrmd320_ltc_di;
...
#define RMD128_CTX CC_RMD128_CTX
#define RMD160_CTX CC_RMD160_CTX
#define RMD256_CTX CC_RMD256_CTX
#define RMD320_CTX CC_RMD320_CTX

#if 0
DIGEST_SHIMS(RMD128, kCCDigestRMD128)
DIGEST_SHIMS(RMD160, kCCDigestRMD160)
DIGEST_SHIMS(RMD256, kCCDigestRMD256)
DIGEST_SHIMS(RMD320, kCCDigestRMD320)
#endif
...
#if 0
int
CC_RMD128_Init(CC_RMD128_CTX *c)
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    const struct ccdigest_info *di = CCDigestGetDigestInfo(kCCDigestRMD128);
    ASSERT(di->state_size == CC_RMD128_DIGEST_LENGTH);
    CC_XZEROMEM(c->hash, CC_RMD128_DIGEST_LENGTH);
    ASSERT(di->block_size == CC_RMD128_BLOCK_BYTES);
    CC_XZEROMEM(c->wbuf, CC_RMD128_BLOCK_BYTES);
    c->count[0] = c->count[1] = 0;
    CC_XMEMCPY(c->hash, di->initial_state, di->state_size);
    return CC_COMPAT_DIGEST_RETURN;
}
...
int
CC_RMD128_Update(CC_RMD128_CTX *c, const void *data, CC_LONG len)
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    return CC_RMD128_Update(c, data, len);
}

int
CC_RMD128_Final(unsigned char *md, CC_RMD128_CTX *c)
{
    uint64_t buf[CC_RMD128_DIGEST_LENGTH/8];
    
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    CC_RMD128_Final((unsigned char *) buf, c);
    CC_XMEMCPY(md, buf, CC_RMD128_DIGEST_LENGTH);
    return CC_COMPAT_DIGEST_RETURN;
}
...
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    const struct ccdigest_info *di = CCDigestGetDigestInfo(kCCDigestRMD160);
    ASSERT(di->state_size == CC_RMD160_DIGEST_LENGTH);
    CC_XZEROMEM(c->hash, CC_RMD160_DIGEST_LENGTH);
    ASSERT(di->block_size == CC_RMD160_BLOCK_BYTES);
    CC_XZEROMEM(c->wbuf, CC_RMD160_BLOCK_BYTES);
    c->count[0] = c->count[1] = 0;
    CC_XMEMCPY(c->hash, di->initial_state, di->state_size);
    return CC_COMPAT_DIGEST_RETURN;
}
...
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    return CC_RMD160_Update(c, data, len);
}

int CC_RMD160_Final(unsigned char *md, CC_RMD160_CTX *c)
{
    uint64_t buf[CC_RMD160_DIGEST_LENGTH/8];
    
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    CC_RMD160_Final((unsigned char *) buf, c);
    CC_XMEMCPY(md, buf, CC_RMD160_DIGEST_LENGTH);
    return CC_COMPAT_DIGEST_RETURN;
}
...
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    const struct ccdigest_info *di = CCDigestGetDigestInfo(kCCDigestRMD256);
    ASSERT(di->state_size == CC_RMD256_DIGEST_LENGTH);
    CC_XZEROMEM(c->hash, CC_RMD256_DIGEST_LENGTH);
    ASSERT(di->block_size == CC_RMD256_BLOCK_BYTES);
    CC_XZEROMEM(c->wbuf, CC_RMD256_BLOCK_BYTES);
    c->count[0] = c->count[1] = 0;
    CC_XMEMCPY(c->hash, di->initial_state, di->state_size);
    return CC_COMPAT_DIGEST_RETURN;
}
...
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    return CC_RMD256_Update(c, data, len);
}

int CC_RMD256_Final(unsigned char *md, CC_RMD256_CTX *c)
{
    uint64_t buf[CC_RMD256_DIGEST_LENGTH/8];
    
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    CC_RMD256_Final((unsigned char *) buf, c);
    CC_XMEMCPY(md, buf, CC_RMD256_DIGEST_LENGTH);
    return CC_COMPAT_DIGEST_RETURN;
}
...
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    const struct ccdigest_info *di = CCDigestGetDigestInfo(kCCDigestRMD320);
    ASSERT(di->state_size == CC_RMD320_DIGEST_LENGTH);
    CC_XZEROMEM(c->hash, CC_RMD320_DIGEST_LENGTH);
    ASSERT(di->block_size == CC_RMD320_BLOCK_BYTES);
    CC_XZEROMEM(c->wbuf, CC_RMD320_BLOCK_BYTES);
    c->count[0] = c->count[1] = 0;
    CC_XMEMCPY(c->hash, di->initial_state, di->state_size);
    return CC_COMPAT_DIGEST_RETURN;
}
...
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    return CC_RMD320_Update(c, data, len);
}

int CC_RMD320_Final(unsigned char *md, CC_RMD320_CTX *c)
{
    uint64_t buf[CC_RMD320_DIGEST_LENGTH/8];
    
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    CC_RMD320_Final((unsigned char *) buf, c);
    CC_XMEMCPY(md, buf, CC_RMD320_DIGEST_LENGTH);
    return CC_COMPAT_DIGEST_RETURN;
}
#endif
