...
CCDHGenerateKey(CCDHRef ref, void *output, size_t *outputLength)
{
    ...
    if(ccdh_generate_key(*(ccdh_gp_t *)&keyref->parms->gp, ccDRBGGetRngState(), *(ccdh_full_ctx_t *)&keyref->ctx.pub))
        return -1;
...
CCDHComputeKey(unsigned char *sharedKey, size_t *sharedKeyLen, const void *peerPubKey, size_t peerPubKeyLen, CCDHRef ref)
{
    ...
    CC_NONULLPARM(ref);
    ccdh_full_ctx_t peer_pub;
    
    CCDH keyref = (CCDH) ref;
    ccdh_pub_ctx_decl_gp(*(ccdh_gp_t *)&keyref->parms->gp, peer_pub);
    cc_size n = ccdh_ctx_n(keyref->ctx);
    cc_unit skey[n];
    
    if(ccdh_import_pub(*(ccdh_gp_t *)&keyref->parms->gp, peerPubKeyLen, peerPubKey,
                    peer_pub))
        return -1;
...
CCDHParametersCreateFromData(const void *p, size_t pLen, const void *g, size_t gLen, size_t l)
{
    ...
    retval->gp = (struct ccdh_const_gp_t *) CC_XMALLOC(retval->malloced);
    if(!retval->gp) goto error;
    if(ccdh_init_gp(retval->gp, n, pval, gval, (cc_size)l))
        goto error;
    return retval;
error:
    if(retval && retval->gp) CC_XFREE((struct ccdh_const_gp_t *) retval->gp, retval->malloced);
...
CCDHParametersRelease(CCDHParameters parameters)
{
    ...
    if(CCDHParm->malloced) {
        CC_XFREE((void *) parameters->gp, retval->malloced);
    }
    CCDHParm->malloced = 0;
    CCDHParm->gp = NULL;
