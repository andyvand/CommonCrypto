...
ccMallocRSACryptor(size_t nbits, CCRSAKeyType keyType)
{
    ...
    ptr = ccrsa_ctx_n(ccrsa_ctx_public(retval->fk));
    ptrb = &ptr;
    *(size_t **)&ptrb = &n;
...
ccRSAkeysize(CCRSACryptor *cryptor) {
    const cc_unit context_m = (const cc_unit)ccrsa_ctx_m(ccrsa_ctx_public(cryptor->fk));
    return ccn_bitlen(ccrsa_ctx_n(ccrsa_ctx_public(cryptor->fk)), &context_m);
}
...
CCRSACryptorRef CCRSACryptorGetPublicKeyFromPrivateKey(CCRSACryptorRef privateCryptorRef)
{
    ...
    ccrsa_init_pub(ccrsa_ctx_public(publicCryptor->fk), ccrsa_ctx_m(ccrsa_ctx_public(privateCryptor->fk)), ccrsa_ctx_e(ccrsa_ctx_public(privateCryptor->fk)));
...
CCCryptorStatus CCRSACryptorImport(const void *keyPackage, size_t keyPackageLen, CCRSACryptorRef *key)
{
    ...
    switch(keyToMake) {
        case ccRSAKeyPublic:
            ctx = *(void **)&ccrsa_ctx_public(cryptor->fk);
            *(void **)&ctx = *(void **)&keyN;
            if(ccrsa_import_pub(cryptor->fk, keyPackageLen, keyPackage)) {
                ccRSACryptorClear(cryptor);
                return kCCDecodeError;
            }
            break;
        case ccRSAKeyPrivate:
            ctx = *(void **)&ccrsa_ctx_private(cryptor->fk);
            *(void **)&ctx = *(void **)&keyN;
...
CCCryptorStatus CCRSACryptorExport(CCRSACryptorRef cryptor, void *out, size_t *outLen)
{
    ...
    switch(cryptor->keyType) {
        case ccRSAKeyPublic:
            bufsiz = ccrsa_export_pub_size(cryptor->fk);
            if(*outLen <= bufsiz) {
                *outLen = bufsiz;
                return kCCBufferTooSmall;
            }
            *outLen = bufsiz;
            if(ccrsa_export_pub(cryptor->fk, bufsiz, out))
                return kCCDecodeError;
            break;
        case ccRSAKeyPrivate:
...
{    
    ...
    cc_size n = ccrsa_ctx_n(ccrsa_ctx_public(rsaKey->fk));
    cc_unit buf[n];
                            ...
                            CCRSACryptorRef *ref)
{
    ...
    __Require_Action(ccn_read_uint(n, (cc_unit *)ccrsa_ctx_m(ccrsa_ctx_public(rsaKey->fk)), modulusLength, modulus) == 0, errOut, retval = kCCParamError);
    __Require_Action(ccn_read_uint(n, (cc_unit *)ccrsa_ctx_m(ccrsa_ctx_public(rsaKey->fk)), exponentLength, exponent) == 0, errOut, retval = kCCParamError);
    cczp_init(ccrsa_ctx_zm(ccrsa_ctx_private(rsaKey->fk)));
    rsaKey->keySize = ccn_bitlen(n, &ccrsa_ctx_m(ccrsa_ctx_public(rsaKey->fk)));
	...
	switch(keyType) {
		...
		case ccRSAKeyPrivate: {
            ...
            CCZP_N(ccrsa_ctx_private_zp(ccrsa_ctx_private(rsaKey->fk))) = psize;
            __Require_Action(ccn_read_uint(psize, CCZP_PRIME(ccrsa_ctx_private_zp(ccrsa_ctx_private(rsaKey->fk))), pLength, p) == 0, errOut, retval = kCCParamError);
            CCZP_N(ccrsa_ctx_private_zq(ccrsa_ctx_private(rsaKey->fk))) = qsize;
            __Require_Action(ccn_read_uint(qsize, CCZP_PRIME(ccrsa_ctx_private_zq(ccrsa_ctx_private(rsaKey->fk))), qLength, q) == 0, errOut, retval = kCCParamError);

            ccrsa_crt_makekey(ccrsa_ctx_zm(ccrsa_ctx_private(rsaKey->fk)), ccrsa_ctx_e(ccrsa_ctx_public(rsaKey->fk)), ccrsa_ctx_d(ccrsa_ctx_public(rsaKey->fk)),
                              ccrsa_ctx_private_zp(ccrsa_ctx_private(rsaKey->fk)),
                              ccrsa_ctx_private_dp(ccrsa_ctx_private(rsaKey->fk)), ccrsa_ctx_private_qinv(ccrsa_ctx_private(rsaKey->fk)),
                              ccrsa_ctx_private_zq(ccrsa_ctx_private(rsaKey->fk)), ccrsa_ctx_private_dq(ccrsa_ctx_private(rsaKey->fk)));
