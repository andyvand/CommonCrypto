...
CCECCryptorGeneratePair(size_t nbits, CCECCryptorRef *publicKey, CCECCryptorRef *privateKey)
{
    ...
    __Require_Action((ccec_generate_key(cp, theRng, *(struct ccec_full_ctx_t *)&privateCryptor->ecKey.private) == 0), errOut, retval = kCCDecodeError);
    ...
    switch(format) {
        case kCCImportKeyBinary: {
            ...
            *keyPackageLen = len;
            ...
            ccec_x963_export(keyType == ccECKeyPrivate, keyPackage, *(void **)&key->ecKey.private);
            break;
        }
        case kCCImportKeyDER:
            retval = kCCUnimplemented;
                    ...
                    void *signedData, size_t *signedDataLen)
{
    ...
    if(ccec_sign(*(struct ccec_full_ctx_t *)&privateKey->ecKey.private, hashSignLen, hashToSign, signedDataLen, signedData, therng) != 0)
                      ...
                      const void *signedData, size_t signedDataLen, uint32_t *valid)
{
    ...
    if(ccec_verify(*(struct ccec_pub_ctx_t *)&publicKey->ecKey.public, hashLen, hash,
...
{
    CCCryptorStatus retval = kCCSuccess;
    void *prk = (void *)privateKey;
    void *puk = (void *)publicKey;
    cc_unit *ounit = (cc_unit *)out;
    
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    if(privateKey == NULL || publicKey == NULL) return kCCParamError;
    if(out == NULL) return kCCParamError;

    if(ccec_compute_key(prk, puk, outLen, ounit)) return kCCDecodeError;
