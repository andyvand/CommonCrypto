...
CCBigNumBitCount(const CCBigNumRef bn)
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    const ccz *s = (ccz *)bn;
    return (uint32_t) ccz_bitlen((ccz *)s);
}
...
CCBigNumZeroLSBCount(const CCBigNumRef bn)
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    const ccz *s = (ccz *)bn;
    return (uint32_t) ccz_trailing_zeros((ccz *)s);
}
...
CCBigNumByteCount(const CCBigNumRef bn)
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    const ccz *s = (ccz *)bn;
    return (uint32_t) ccz_write_uint_size((ccz *)s);
}
...
CCBigNumToData(CCStatus *status, const CCBigNumRef bn, void *to)
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    const ccz *s = (ccz *)bn;
    size_t to_size = ccz_write_uint_size((ccz *)s);
    ccz_write_uint((ccz *)s, to_size, to);
    return to_size;
}
...
CCBigNumFromHexString(CCStatus *status, const char *in)
{
    ...
    ccz *r = (ccz *)CCCreateBigNum(status);
    if (r) {
        if (ccz_read_radix(r, strlen(in), (void *)in, 16)) {
            ccz_zero((struct ccz_class *)r);
            if (status)
...
CCBigNumToHexString(CCStatus *status, const CCBigNumRef bn)
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    const ccz *s = (ccz *)bn;
    size_t to_size = ccz_write_radix_size((ccz *)s, 16);
    char *to = malloc(to_size+1);
    ccz_write_radix((ccz *)s, to_size, to, 16);
    to[to_size] = 0;
    return to;
}
...
CCBigNumFromDecimalString(CCStatus *status, const char *in)
{
    ...
    ccz *r = (ccz *)CCCreateBigNum(status);
    if (r) {
        if (ccz_read_radix(r, strlen(in), (void *)in, 10)) {
            ccz_zero((struct ccz_class *)r);
            if (status)
                *status = kCCDecodeError;
            return NULL;
...
CCBigNumToDecimalString(CCStatus *status, const CCBigNumRef bn)
{
    CC_DEBUG_LOG(ASL_LEVEL_ERR, "Entering\n");
    const ccz *s = (ccz *)bn;
    size_t to_size = ccz_write_radix_size((ccz *)s, 10);
    char *to = malloc(to_size+1);
    ccz_write_radix((ccz *)s, to_size, to, 10);
    to[to_size] = 0;
    return to;
}
